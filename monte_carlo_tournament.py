...
def get_cached_prediction(home, away, stage="Group Stage"):
    ...
    if _CATBOOST_HOME is None:
        _CATBOOST_HOME = CatBoostRegressor().load_model(os.path.join("models", "saved", "nexus_home.cbm"))
        _CATBOOST_AWAY = CatBoostRegressor().load_model(os.path.join("models", "saved", "nexus_away.cbm"))
...
def run_monte_carlo(iterations=1000):
    ...
    print("Saved Monte Carlo results to monte_carlo_results.json")
